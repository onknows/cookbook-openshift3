...
Chef::Log.warn("The CLUSTER HOSTED CERTS redeploy will be skipped on First master. Could not find the flag: #{node['is_apaas_openshift_cookbook']['redeploy_cluster_hosted_certserver_control_flag']}") unless ::File.file?(node['is_apaas_openshift_cookbook']['redeploy_cluster_hosted_certserver_control_flag'])

if ::File.file?(node['is_apaas_openshift_cookbook']['redeploy_cluster_hosted_certserver_control_flag'])
  ...
  if node['is_apaas_openshift_cookbook']['openshift_hosted_manage_registry']
    execute 'Re-Generate Cluster CA certificates for Hosted Registry' do
      command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} env dc/docker-registry OPENSHIFT_CA_DATA=\"$(cat ca.crt)\" OPENSHIFT_CERT_DATA=\"$(cat openshift-registry.crt)\" OPENSHIFT_KEY_DATA=\"$(cat openshift-registry.key)\" -n ${namespace_registry} --config=admin.kubeconfig"
      environment(
      ...
      only_if "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} env dc/docker-registry --list -n ${namespace_registry} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig | grep OPENSHIFT_CA_DATA"
    ...
    execute 'Redeploy docker registry' do
      command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} deploy dc/docker-registry --latest -n ${namespace_registry} --config=admin.kubeconfig"
      environment(
        'namespace_registry' => node['is_apaas_openshift_cookbook']['openshift_hosted_registry_namespace']
      )
      retries 2
  ...
  if node['is_apaas_openshift_cookbook']['openshift_hosted_manage_router']
    execute 'Re-Generate Cluster CA certificates for Hosted Router' do
      command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} env dc/router OPENSHIFT_CA_DATA=\"$(cat ca.crt)\" OPENSHIFT_CERT_DATA=\"$(cat openshift-router.crt)\" OPENSHIFT_KEY_DATA=\"$(cat openshift-router.key)\" -n ${namespace_router} --config=admin.kubeconfig"
      environment(
        'namespace_router' => node['is_apaas_openshift_cookbook']['openshift_hosted_router_namespace']
      )
      cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
      only_if "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} env dc/router --list -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig | grep OPENSHIFT_CA_DATA"
      notifies :run, 'execute[Wait for 10 seconds whilst updating ENV]', :immediately
    end
    ...
    execute 'Remove router service annotations' do
      command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} annotate service/router service.alpha.openshift.io/serving-cert-secret-name- service.alpha.openshift.io/serving-cert-signed-by- -n ${namespace_router}"
      environment(
        'namespace_router' => node['is_apaas_openshift_cookbook']['openshift_hosted_router_namespace']
      )
    end
    ...
    execute 'Add serving-cert-secret annotation to router service' do
      command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} annotate service/router service.alpha.openshift.io/serving-cert-secret-name=router-certs -n ${namespace_router}"
      environment(
        'namespace_router' => node['is_apaas_openshift_cookbook']['openshift_hosted_router_namespace']
      )
    end
    ...
    execute 'Create Hosted Router Certificate' do
      command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} secrets new router-certs --type=kubernetes.io/tls tls.crt=${certfile} tls.key=${keyfile} -n ${namespace_router} --confirm"
      environment(
    ...
    execute 'Redeploy router' do
      command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} deploy dc/router --latest -n ${namespace_router} --config=admin.kubeconfig"
      environment(
        'namespace_router' => node['is_apaas_openshift_cookbook']['openshift_hosted_router_namespace']
      )
      retries 2
  ...
  file node['is_apaas_openshift_cookbook']['redeploy_cluster_hosted_certserver_control_flag'] do
    action :delete
  end
end
