...
if node['cookbook-openshift3']['adhoc_redeploy_cluster_ca']
  ...
  certs_to_be_removed = case ose_major_version.split('.')[1].to_i
                        when 5..7
                          node['cookbook-openshift3']['openshift_master_certs'] + %w(service-signer.crt service-signer.key) + %w(ca.crt ca.key ca.serial.txt ca-bundle.crt master.server.crt master.server.key openshift-master.crt openshift-master.key openshift-master.kubeconfig)
                        else
                          node['cookbook-openshift3']['openshift_master_certs'] + %w(ca.crt ca.key ca.serial.txt ca-bundle.crt master.server.crt master.server.key openshift-master.crt openshift-master.key openshift-master.kubeconfig)
                        end

  certs_to_be_removed.each do |remove_certificate|
    file "#{node['cookbook-openshift3']['openshift_master_config_dir']}/#{remove_certificate}" do
      action :delete
