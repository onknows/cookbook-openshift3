...
Chef::Log.error("The ETCD CA certificate redeploy will be skipped. Could not find the flag: #{node['is_apaas_openshift_cookbook']['redeploy_etcd_ca_control_flag']}") unless ::File.file?(node['is_apaas_openshift_cookbook']['redeploy_etcd_ca_control_flag'])
...
if ::File.file?(node['is_apaas_openshift_cookbook']['redeploy_etcd_ca_control_flag'])
  ...
  if is_certificate_server
    if ::File.directory?(node['is_apaas_openshift_cookbook']['etcd_ca_dir']) && !::File.exist?("#{node['is_apaas_openshift_cookbook']['etcd_ca_dir']}/ca-bundle.crt")
      helper.backup_dir(node['is_apaas_openshift_cookbook']['etcd_ca_dir'], "#{node['is_apaas_openshift_cookbook']['etcd_ca_dir']}-#{Time.now.strftime('%Y-%m%d-%H%M')}")
    ...
    ruby_block 'Create ETCD CA Bundle' do
      block do
        helper.bundle_etcd_ca(["#{node['is_apaas_openshift_cookbook']['etcd_ca_dir']}/ca.crt", '/var/www/html/etcd/ca.crt'], "#{node['is_apaas_openshift_cookbook']['etcd_ca_dir']}/ca-bundle.crt")
      end
  ...
  if is_etcd_server
    remote_file "#{node['is_apaas_openshift_cookbook']['etcd_conf_dir']}/ca.crt" do
      source "http://#{certificate_server['ipaddress']}:#{node['is_apaas_openshift_cookbook']['httpd_xfer_port']}/etcd/ca-bundle.crt"
      sensitive true
      retries 15
      retry_delay 2
    ...
    execute 'Check cluster health with new CA bundle' do
      command "/usr/bin/etcdctl --cert-file #{node['is_apaas_openshift_cookbook']['etcd_peer_file']} --key-file #{node['is_apaas_openshift_cookbook']['etcd_peer_key']} --ca-file #{node['is_apaas_openshift_cookbook']['etcd_ca_cert']} -C https://`hostname`:2379 member list"
