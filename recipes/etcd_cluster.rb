...
if is_etcd_server || is_certificate_server
  yum_package 'etcd' do
    ...
    options '---exclude=etcd-3.2*' if ose_major_version.split('.')[1].to_i < 6
    ...
    not_if { `rpm -q etcd` && (node['cookbook-openshift3']['etcd_version'].to_s.split('-')[0]).to_s < `repoquery --plugins --installed --qf '%{version}' etcd` && ose_major_version.split('.')[1].to_i < 6 }
  end
end
