...
unless etcd_servers.size == 1
  if is_certificate_server
    ...
    execute 'Check ETCD cluster readiness' do
      command "/usr/bin/etcdctl --cert-file #{node['cookbook-openshift3']['etcd_generated_certs_dir']}/etcd-#{first_etcd['fqdn']}/peer.crt --key-file #{node['cookbook-openshift3']['etcd_generated_certs_dir']}/etcd-#{first_etcd['fqdn']}/peer.key --ca-file #{node['etcd_generated_ca_dir']}/ca.crt -C https://#{first_etcd['ipaddress']}:2379 get /rollback/etcd | grep -w ready"
      retries 120
      retry_delay 5
    end
