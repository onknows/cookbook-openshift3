...
if is_etcd_server
  ...
  node.run_state['issues_detected'] = true unless Dir["#{node['cookbook-openshift3']['etcd_data_dir']}/member/snap/*.snap"].any?
  ...
  node.run_state['issues_detected'] = true if Dir["#{Chef::Config[:file_cache_path]}/etcd_migration-fail"]
