...
action :create do
  converge_by "Deploying Metrics on #{node['fqdn']}" do
    ...
    ruby_block 'Create Routes' do
      block do
        [{ 'metadata' => { 'name' => 'hawkular-metrics', 'labels' => { 'metrics-infra' => 'hawkular-metrics' } }, 'spec' => { 'host' => node['is_apaas_openshift_cookbook']['openshift_metrics_hawkular_hostname'], 'to' => { 'kind' => 'Service', 'name' => 'hawkular-metrics' }, 'tls' => { 'termination' => 'reencrypt', 'destinationCACertificate' => ::File.read("#{FOLDER}/ca.crt") } } }].each do |route|
    ...
    %w(hawkular-cassandra-1 hawkular-metrics heapster).each do |rc|
      ruby_block "Check existing RC #{rc}" do
        block do
          require 'fileutils'
          get_rc_status = Mixlib::ShellOut.new("#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get rc #{rc} -o yaml --namespace=#{node['is_apaas_openshift_cookbook']['openshift_metrics_project']} --config=#{FOLDER}/admin.kubeconfig").run_command.stdout.strip
          get_rc_readyness = Mixlib::ShellOut.new("#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get rc #{rc} -o yaml --namespace=#{node['is_apaas_openshift_cookbook']['openshift_metrics_project']} --config=#{FOLDER}/admin.kubeconfig").run_command.stdout.strip
        ...
        only_if "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get rc #{rc} --namespace=#{node['is_apaas_openshift_cookbook']['openshift_metrics_project']} --config=#{FOLDER}/admin.kubeconfig"
      end
    end
